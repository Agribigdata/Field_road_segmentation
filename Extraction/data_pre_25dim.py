    ...
    for i in range(len(files)):
        ...
        for j in range(len(list(speed_list))):
            if j == 0:
                ...
                try:
                    d1 = datetime.datetime.strptime(str(time_list[j - 1]),
                                                    '%Y/%m/%d %H:%M:%S')
                    d2 = datetime.datetime.strptime(str(time_list[j]),
                                                    '%Y/%m/%d %H:%M:%S')
                except:
                    d1 = datetime.datetime.strptime(str(time_list[j - 1]),
                                                    '%Y-%m-%d %H:%M:%S')
        ...
        for temp in range(len(list(speed_list))):
            if temp == 0:
                ...
                if speed_list[temp] == 0:
                    ...
                    angular_speed[temp] = angular_list[temp] / speed_list[temp]
        for k in range(len(list(speed_list))):
            if k == 0:
                ...
                try:
                    d1 = datetime.datetime.strptime(str(time_list[k - 1]),
                                                    '%Y/%m/%d %H:%M:%S')
                    d2 = datetime.datetime.strptime(str(time_list[k]),
                                                    '%Y/%m/%d %H:%M:%S')
                except:
                    d1 = datetime.datetime.strptime(str(time_list[k - 1]),
                                                    '%Y-%m-%d %H:%M:%S')
        ...
        columns = [
            'lon', 'lat', 'speed', 'acceleration', 'angular_speed',
            'angular_acceleration', 'angle_diff', 'tag'
        ]
...
if __name__ == "__main__":
    ...
    for q in range(len(allfile)):
        ...
        try:
            data2 = data1.loc[:, [
                'time', 'longitude', 'latitude', 'speed', 'dir', 'tags'
            ]]
            x = data2['longitude']
            y = data2['latitude']
            speed = data2['speed']
    ...
    for i in files:
        ...
        columns = [
            'speed', 'speed_med_5', 'speed_med_20', 'speed_SD_5',
            'speed_SD_20', 'acceleration', 'acceleration_med_5',
            'acceleration_med_20', 'acceleration_SD_5', 'acceleration_SD_20',
            'angular_speed', 'angular_speed_med_5', 'angular_speed_med_20',
            'angular_speed_SD_5', 'angular_speed_SD_20',
            'angular_acceleration', 'angular_acceleration_med_5',
            'angular_acceleration_med_20', 'angular_acceleration_SD_5',
            'angular_acceleration_SD_20', 'angle_diff', 'angle_diff_med_5',
            'angle_diff_med_20', 'angle_diff_SD_5', 'angle_diff_SD_20', 'tag',
            'lon', 'lat'
        ]
        df = pd.DataFrame(columns=columns)
        df["speed"] = speed_list
