...
def get_d1(path, result_excel, allfileq):
    data_list_lon = result_excel['lon'].tolist()
    data_list_lat = result_excel['lat'].tolist()
    temp_lon_lat = zip(data_list_lon, data_list_lat)
    list_all = list(temp_lon_lat)

    key_dis_list = []

    all_min_points_dis = []
    all_second_min_points_dis = []
    temp_min_points_dis = []
    temp_second_min_points_dis = []
    for key in range(len(list_all)):
        for key_then in range(len(list_all)):
            if key_then == key or key_then == key + 1 or key_then == key - 1:
                continue
            else:
                d = geodistance(list_all[key], list_all[key_then])
                heapq.heappush(key_dis_list, (d, key_then))

        temp_min_points_dis.append(key)
        item_min=heapq.heappop(key_dis_list)
        temp_min_points_dis.append(item_min[1])
        temp_min_points_dis.append(item_min[0])
        all_min_points_dis.append(temp_min_points_dis)

        temp_second_min_points_dis.append(key)
        item_min2 = heapq.heappop(key_dis_list)
        temp_second_min_points_dis.append(item_min2[1])
        temp_second_min_points_dis.append(item_min2[0])
        all_second_min_points_dis.append(temp_second_min_points_dis)
        temp_second_min_points_dis = []
        temp_min_points_dis = []

        all_min_points_dis_to_csv = pd.DataFrame(all_min_points_dis)
        all_min_points_dis_to_csv.rename(columns={0: 'begin_point', 1: 'end_point', 2: "distance"}, inplace=True)

        all_second_min_points_dis_to_csv = pd.DataFrame(all_second_min_points_dis)
        all_second_min_points_dis_to_csv.rename(columns={0: 'begin_point', 1: 'end_point', 2: "distance"}, inplace=True)

        all_min_points_dis_to_csv.to_pickle(path + "点关系/first/" + allfileq.split('.')[0] + ".xlsx")
        all_second_min_points_dis_to_csv.to_pickle( path + "点关系/second/" + allfileq.split('.')[0] + ".xlsx")




        key_dis_list=[]
def get_t12(path,result,allfileq):
    data=result
    ...
    for j in range(len(data_list_lon)):
        ...
        if j != len(data_list_lon) - 2 and j != len(data_list_lon) - 1:
            ...
            d_begintomiddle = geodistance([data_list_lon[begin_point], data_list_lat[begin_point]],
                                          [data_list_lon[middle_point], data_list_lat[middle_point]])
            d_middletoend = geodistance([data_list_lon[middle_point], data_list_lat[middle_point]],
            ...
            distance_begintomiddle.append(d_begintomiddle)
            distance_middletoend.append(d_middletoend)
    all_oneandtwo_points_dis_to_csv = pd.DataFrame(
        {'begin_point': t_1andt_2begin_point, 'middle_point': t_1andt_2middle_point,
         'end_point': t_1andt_2end_point, 'begintomiddle_distance': distance_begintomiddle,
         'middletoend_distance': distance_middletoend})

    all_oneandtwo_points_dis_to_csv.to_pickle(
        path + "点关系/onedisandtwodis_down/" + allfileq.split('.')[0] + ".xlsx"
        )
    t_1andt_2begin_point = []
    t_1andt_2middle_point = []
    t_1andt_2end_point = []
    ...
    all_oneandtwo_points_dis_to_csv.to_pickle(
        path + "点关系/onedisandtwodis_up/" + allfileq.split('.')[0] + ".xlsx")
